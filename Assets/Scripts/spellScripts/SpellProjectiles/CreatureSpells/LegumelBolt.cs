...
public class LegumelBolt : MonoBehaviour {
    ...
    private GameObject thisLegumelBolt;
    ...
    void Start() {
        ...
        thisLegumelBolt = this.gameObject;
        corePs = thisLegumelBolt.transform.GetChild(0).GetComponent<ParticleSystem>();
        trailPs = thisLegumelBolt.transform.GetChild(1).GetComponent<ParticleSystem>();
    ...
    private void OnCollisionEnter2D(Collision2D col) {
        ...
        if (col.gameObject.tag == "Creature") return;
        
        if (col.gameObject.name == "Player") {
            var playerScript = col.gameObject.GetComponent<Player>();
            if (!damageDealt) {
                damageDealt = true;
