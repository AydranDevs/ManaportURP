    ...
    namespace MirabelleNamespace {
        public class MirabellePartyInput : MonoBehaviour {
            ...
            private void RecalcPath() {
                ...
                path = Pathfinding.FindPath_Static(grid, mirabelle.transform.position, leaderTransform.position);
                goalReached = false;
                pathIndex = 0;
            }
            ...
            private void PathfindingUpdate() {
                ...
                if (Vector2.Distance(mirabelle.transform.position, leaderTransform.position) > party.maxDistance) {
                ...
                if (grid.WorldPositionToTile(mirabelle.transform.position) != path[pathIndex]) {
            ...
            private void MoveToTile(WorldTile tile) {
                WorldTile currentTile = grid.WorldPositionToTile(mirabelle.transform.position);
