    ...
    public class Spindash : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (laurie.auxilaryMovementType == AuxilaryMovementType.Spindash && laurie.abilityState == AbilityState.AuxilaryMovement)
            {
                ...
                float range = laurie.stats.abilityDistances[0].value;
                ...
                if (time <= 0f)
                {
                    ...
                    laurieAbilities.abilityCooldown = laurie.stats.abilityCooldownLimit.value;
                    time = laurie.stats.abilityDistances[0].value * 0.1f;
